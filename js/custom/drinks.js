...
// No longer in use, but we may bring it back next winter!
...
var canvas = document.getElementById('snow'),
  ...
  height = canvas.height = document.getElementById('snow-height').offsetHeight,
...
function init() {
  ...
  while (num_drops--) {
    ...
    drop.vy = Math.random() * drop.speed + (drop.speed / 2);
	     drop.radius = (drop.distance + 1) / 1 * 3;

	// snow size changes depending on browser width via jpb
	if (width > 960)
	  {
	  ...
	  {
	     drop.radius = (drop.distance + 1) / 45 * 3;
	  }
